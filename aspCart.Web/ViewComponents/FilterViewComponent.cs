...
namespace aspCart.Web.ViewComponents
{
    ...
    public class FilterViewComponent : ViewComponent
    {
        ...
        public IViewComponentResult Invoke(string nameFilter = "", string categoryFilter = "", string manufacturerFilter = "" )
        {
            ...
            {
                var filterModel = new CategoryFilterViewModel { SeoUrl = category.SeoUrl, Name = category.Name, Id = category.Id };
                var qty = 0;
                ...
                if(manufacturerFilter.Length > 0)
                {
                    qty = filterModel.Quantity = _productService.Table()
                        ...
                        .Where(x => x.Manufacturers.Any(m => m.Manufacturer.SeoUrl.ToLower() == manufacturerFilter.ToLower()))
                        .Where(x => x.Categories.Any(c => c.Category.SeoUrl.ToLower() == category.SeoUrl.ToLower()))
                        .Where(x => x.Published == true)
                        .AsNoTracking()
                        .Count();
            ...
            {
                var filterModel = new ManufacturerFilterViewModel { SeoUrl = manufacturer.SeoUrl, Name = manufacturer.Name, Id = manufacturer.Id };
                var qty = 0;

                qty = filterModel.Quantity = _productService.Table()
                    .Include(x => x.Categories).ThenInclude(x => x.Category)
                    .Include(x => x.Manufacturers).ThenInclude(x => x.Manufacturer)
                    .Where(x => x.Manufacturers.Any(m => m.Manufacturer.SeoUrl.ToLower() == manufacturer.SeoUrl.ToLower()))
                    .Where(x => x.Categories.Any(c => c.Category.SeoUrl.ToLower() == categoryFilter.ToLower()))
                    .Where(x => x.Published == true)
                    .AsNoTracking()
                    .Count();
            ...
            {
                allProducts = allProducts
                    .Where(x => x.Categories.Any(m => m.Category.SeoUrl.ToLower() == categoryFilter.ToLower()))
                    .ToList();
